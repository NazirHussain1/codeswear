...
export default function Post({ addToCart }) {
  ...
  const [pin, setPin] = useState(""); // start with empty string
  ...
  const CheckService = async () => {
    ...
    try {
      ...
      // support both: []  or { pincodes: [] }
      const pinsArray = Array.isArray(data) ? data : Array.isArray(data.pincodes) ? data.pincodes : [];
      ...
      setService(false); // treat errors as not serviceable
    }
  };
            ...
            <div className="lg:w-1/2 w-full lg:pl-8 lg:py-6 mt-6 lg:mt-0">
              ...
              <div className="flex mb-4">
                <span className="flex items-center">
                  {/* NOTE: SVG props use camelCase for React */}
              ...
              <div className="flex">
               ...
               <div className="flex mt-1 space-x-4 ms-0 ">
                ...
                <button
                  onClick={() => {
                    // ensure parameters are in correct order: (itemCode, qty, name, price, size, varient)
                    if (!slug) {
                      // optional: guard if slug not yet ready
                      console.warn("slug not ready");
                      return;
                    }
                    addToCart(slug, 1, 5999, "wear the code (XL, red)", "XL", "red");
                  }}
                  disabled={!slug}
              ...
              {Service === false && (
                <div className="text-red-700 text-sm mt-3">Sorry! We cannot deliver at that point</div>
              )}

              {Service === true && (
                <div className="text-green-700 text-sm mt-3">Yes! We can deliver at that point</div>
              )}
            </div>
          </div>
        </div>
