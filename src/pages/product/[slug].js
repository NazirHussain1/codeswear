import { useRouter } from "next/router";
import { useState } from "react";
import Product from "@/models/product";
import { connectDB } from "@/lib/db";

export async function getServerSideProps(context) {
  const { slug } = context.params;

  await connectDB();

  const product = await Product.findOne({ slug }).lean();

  if (!product) {
    return { notFound: true };
  }

  const variants = await Product.find({ title: product.title }).lean();

    let colorSizeSlug = {};
  for (let item of variants) {
    
    const colors = Array.isArray(item.color) ? item.color : [item.color];
    const sizes = Array.isArray(item.size) ? item.size : [item.size];

    colors.forEach((color) => {
      sizes.forEach((size) => {
        if (!colorSizeSlug[color]) {
          colorSizeSlug[color] = {};
        }
        colorSizeSlug[color][size] = { slug: item.slug };
      });
    });
  }

  console.log("Processed variants:", colorSizeSlug);

  return {
    props: {
      product: JSON.parse(JSON.stringify(product)),
      variants: JSON.parse(JSON.stringify(colorSizeSlug)),
    },
  };
}

export default function Post({ buyNow,addToCart, product, variants }) {
  console.log("Product:", product);
  console.log("Variants:", variants);

  const router = useRouter();
  const { slug } = router.query;
  const [service, setService] = useState();
  const [pin, setPin] = useState("");

   const getInitialColor = () => {

    if (Array.isArray(product.color) && product.color.length > 0) {
      const firstColor = product.color[0];
 
      if (variants[firstColor]) {
        return firstColor;
      }
    }

      const availableColors = Object.keys(variants);
    if (availableColors.length > 0) {
      return availableColors[0];
    }

    return ""; 
  };

  const getInitialSize = () => {
    const initialColor = getInitialColor();

        if (initialColor && variants[initialColor]) {
      const availableSizes = Object.keys(variants[initialColor]);
      if (availableSizes.length > 0) {
        return availableSizes[0];
      }
    }

    return "M";
  };

  const [color, setColor] = useState(getInitialColor);
  const [size, setSize] = useState(getInitialSize);

  const CheckService = async () => {
    if (!pin) {
      setService(false);
      return;
    }
    const pinNum = parseInt(pin, 10);
    if (Number.isNaN(pinNum)) {
      setService(false);
      return;
    }

    try {
      const res = await fetch("/api/pincode");
      if (!res.ok) throw new Error(`Network error: ${res.status}`);
      const data = await res.json();

      
      const pinsArray = Array.isArray(data)
        ? data
        : Array.isArray(data.pincodes)
        ? data.pincodes
        : [];

      setService(pinsArray.includes(pinNum));
      console.log("available pins:", pinsArray, "checked:", pinNum);
    } catch (error) {
      console.error("CheckService error:", error);
      setService(false); 
    }
  };

  
  const onPinChange = (e) => {
    setPin(e.target.value);

    if (service !== undefined) setService(undefined);
  };


  const getAvailableColors = () => {
    if (!variants || typeof variants !== "object") return [];
    return Object.keys(variants).filter(
      (color) => variants[color] && variants[color][size]
    );
  };

  const getAvailableSizes = () => {
    if (!variants || !variants[color]) return [];
    return Object.keys(variants[color]);
  };


  const refreshVariant = (newSize, newColor) => {
    const selectedSize = newSize || size;
    const selectedColor = newColor || color;


    if (variants[selectedColor] && variants[selectedColor][selectedSize]) {
      const newSlug = variants[selectedColor][selectedSize].slug;

      setSize(selectedSize);
      setColor(selectedColor);

      if (newSlug !== slug) {
        router.push(`/product/${newSlug}`);
      }
    } else {
      console.warn(`Variant not found: ${selectedColor} - ${selectedSize}`);
    }
  };

  return (
    <>
      <section className="text-gray-600 body-font overflow-hidden">
        <div className="container px-4 py-8 mx-auto">
          <div className="lg:w-4/5 flex flex-wrap">
            <img
              alt="ecommerce"
              className="lg:w-1/2 w-full lg:h-auto px-24 object-cover object-top rounded"
              src={
                product.img ||
                "https://m.media-amazon.com/images/I/81YtgFYwxAL._AC_SX466_.jpg"
              }
            />
            <div className="lg:w-1/2 w-full lg:pl-8 lg:py-6 mt-6 lg:mt-0">
              <h2 className="text-sm title-font text-gray-500 tracking-widest">
                CODES WEAR
              </h2>
         
              <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">
               
                {product.title} ({size}/{color})
              </h1>

              <div className="flex mb-4">
                {/* <span className="flex items-center"> */}
                  {/* Star ratings */}
                  {/* {[1, 2, 3, 4].map((star) => ( */}
                    {/* <svg
                      key={star}
                      fill="currentColor"
                      stroke="currentColor"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2"
                      className="w-4 h-4 text-pink-500"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                    </svg>
                  ))}
                  <svg
                    fill="none"
                    stroke="currentColor"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    className="w-4 h-4 text-pink-500"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                  </svg>
                  <span className="text-gray-600 ml-3">4 Reviews</span>
                </span> */}

                {/* <span className="flex ml-3 pl-3 py-2 border-l-2 border-gray-200 space-x-2s">
                  <a className="text-gray-500">
                    <svg
                      fill="currentColor"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2"
                      className="w-5 h-5"
                      viewBox="0 0 24 24"
                    >
                      <path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z" />
                    </svg>
                  </a>
                  <a className="text-gray-500">
                    <svg
                      fill="currentColor"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2"
                      className="w-5 h-5"
                      viewBox="0 0 24 24"
                    >
                      <path d="M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2c9 5 20 0 20-11.5a4.5 4.5 0 00-.08-.83A7.72 7.72 0 0023 3z" />
                    </svg>
                  </a>
                  <a className="text-gray-500">
                    <svg
                      fill="currentColor"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2"
                      className="w-5 h-5"
                      viewBox="0 0 24 24"
                    >
                      <path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z" />
                    </svg>
                  </a>
                </span> */}
              </div>

             
              <p className="leading-relaxed">
                {product.desc }
              </p>

              <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                <div className="flex items-center">
                  <span className="mr-3">Color</span>

             
                  {getAvailableColors().map((colorOption) => (
                    <button
                      key={colorOption}
                      onClick={() => refreshVariant(size, colorOption)}
                      className={`border-2 ml-1 rounded-full w-6 h-6 focus:outline-none ${
                        color === colorOption
                          ? "border-black"
                          : "border-gray-300"
                      } ${
                        colorOption === "White"
                          ? "bg-white"
                          : colorOption === "Blue"
                          ? "bg-blue-600"
                          : colorOption === "Purple"
                          ? "bg-purple-600"
                          : colorOption === "Yellow"
                          ? "bg-yellow-400"
                          : colorOption === "Green"
                          ? "bg-green-600"
                          : colorOption === "Red"
                          ? "bg-red-600"
                          : "bg-gray-300"
                      }`}
                    ></button>
                  ))}
                </div>

                <div className="flex ml-6 items-center">
                  <span className="mr-3">Size</span>
                  <div className="relative">
                    <select
                      value={size}
                      onChange={(e) => refreshVariant(e.target.value, color)}
                      className="rounded border appearance-none border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-pink-200 focus:border-pink-500 text-base pl-3 pr-10"
                    >
                     
                      {getAvailableSizes().map((sizeOption) => (
                        <option key={sizeOption} value={sizeOption}>
                          {sizeOption}
                        </option>
                      ))}
                    </select>
                    <span className="absolute right-0 top-0 h-full w-10 text-center text-gray-600 pointer-events-none flex items-center justify-center">
                      <svg
                        fill="none"
                        stroke="currentColor"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth="2"
                        className="w-4 h-4"
                        viewBox="0 0 24 24"
                      >
                        <path d="M6 9l6 6 6-6" />
                      </svg>
                    </span>
                  </div>
                </div>
              </div>

              <div className="flex">
           
                <span className="title-font font-medium text-2xl text-gray-900">
                  ₹{product.price}.00
                </span>

                <button className="rounded-full w-10 h-10 bg-gray-200 p-0 border-0 inline-flex items-center justify-center text-gray-500 ms-16">
                  <svg
                    fill="currentColor"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    className="w-5 h-5"
                    viewBox="0 0 24 24"
                  >
                    <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z" />
                  </svg>
                </button>
              </div>

              <div className="flex mt-1 space-x-4 ms-0">
                <button  onClick={() => {buyNow(
                      slug,
                      1,
                      product.price || 5999,
                      `${product.title} (${size}/ ${color})`,
                      size,
                      color
                    );}} className="flex ml-4 text-white bg-pink-500 border-0 md:py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded">
                  Buy Now
                </button>

                <button
                  onClick={() => {
                    if (!slug) {
                      console.warn("slug not ready");
                      return;
                    }
                    
                     addToCart(
                      slug,
                      1,
                      product.price || 5999,
                      `${product.title} (${size}/ ${color})`,
                      size,
                      color
                    );
                  }}
                  disabled={!slug}
                  className="flex ml-2 text-white bg-pink-500 border-0 md:py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded disabled:opacity-50"
                >
                  Add to Cart
                </button>
              </div>

              <div className="flex mt-3 space-x-2">
                <input
                  onChange={onPinChange}
                  value={pin}
                  type="text"
                  placeholder="Enter your City Pincode"
                  className="px-3 py-2 rounded border border-gray-200"
                  maxLength={6}
                />
                <button
                  onClick={CheckService}
                  className="text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded"
                >
                  Check
                </button>
              </div>

              {service === false && (
                <div className="text-red-700 text-sm mt-3">
                  Sorry! We cannot deliver to this pincode
                </div>
              )}

              {service === true && (
                <div className="text-green-700 text-sm mt-3">
                  Yes! We deliver to this pincode
                </div>
              )}
            </div>
          </div>
        </div>
      </section>
    </>
  );
}
